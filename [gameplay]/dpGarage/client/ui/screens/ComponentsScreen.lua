...
function ComponentsScreen:resetComponent()
	if self.currentComponent and self.componentName and self.componentPosition then
		self.vehicle:setComponentPosition(self.componentName, unpack(self.componentPosition))
	end
end
...
function ComponentsScreen:update(deltaTime)
	...
	if self.currentComponent then
		local offsetMul = math.sin(getTickCount() / 200)
...
function ComponentsScreen:onKey(key)
	...
	if key == "arrow_r" then
		ComponentsScreen:resetComponent()
